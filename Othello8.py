...
def GenerateDecompressionTable():#returns a decompression table for DecompressOpeningBook
    table = dict()
    ps = [3,3**2,3**3,3**4,3**5,3**6,3**7,3**8]
    cs = ['.','x','o']
    for i in range(3**8):
        table[chr(i)] = f'{cs[i%ps[7]]}{cs[i%ps[6]]}{cs[i%ps[5]]}{cs[i%ps[4]]}{cs[i%ps[3]]}{cs[i%ps[2]]}{cs[i%ps[1]]}{cs[i%ps[0]]}'
    return table

def DecompressOpeningBook():#decodes from base 6561 to 8 base 3 characters, 8x board compression
    global OPENING_BOOK
    decode_table = GenerateDecompressionTable()
    new_opening_book = dict()
    for compressed_board, moves in OPENING_BOOK.items():
        new_opening_book[decode_table[compressed_board]] = moves
...
def ScoreBoardHeuristic(board, token):#attempts to play off of safe positions into safe positions. improvement could be to aggressively seek out safe positions when not having any. agggressive play to get corners would also be good
    ...
    score += (len(token_pos.intersection(CORNERS)) - len(other_token_pos.intersection(CORNERS))) * 50#score for corner possession and loss of score for enemy corner position
    score -= (sum([board[s[1][0]] == token + board[s[1][1]] == token for s in NEXT_TO_CORNERS.items() if board[s[0]] == '.']) - sum([board[s[1][0]] == other_token + board[s[1][1]] == other_token for s in NEXT_TO_CORNERS.items() if board[s[0]] == '.'])) * 100
    ...
    score += (len(token_moves) - len(other_token_moves)) * 10#attempt to minimize enemy moves and maximize my own
    ...
    score += len(SAFE_POSITIONS) * 30#attempt to get stable areas
...
def alphabeta(board, token, alpha, beta, depth=-1):
    if depth == 0:#should return up now
...
def main():
    #random.seed(1)
    global args, DEBUG, count, LIMIT_AB
    GenGlobals()
    ...
    #args = ['...........................ox......xxx..........................', 'o']
    #args = ['.' * 27 + "ox......xo" + '.' * 27, 'x']
    #moves = '193441212920113722121326181030_438_3_2_1_5_6_92517_0334032_81624-14948565758-150-1424351-114-12315_7-1473931-14554-354'.replace('_', '0')
    #moves = [moves[i: i + 2] for i in range(0, len(moves), 2)]
    #moves = list(map(int, moves))
    #playGame(moves=moves, token='o')
    #return
    playback_x = {4: 19, 6: 26, 8: 11, 10: 12, 12: 17, 14: 3, 16: 32, 18: 1, 20: 4, 22: 5, 24: 22, 26: 23, 28: 39, 30: 7, 31: 42, 33: 15, 35: 0, 36: 8, 37: 24, 38: 40, 39: 21, 40: 29, 41: 37, 42: 44}  # moves from a game of pcgabor vs edax where these are edax's moves
    playback_o = {5: 34, 7: 21, 9: 20, 11: 37, 13: 12, 15: 26, 17: 10, 19: 4, 21: 3, 23: 1, 25: 6, 27: 25, 29: 0, 31: 40, 33: 8, 35: 24, 36: 49, 38: 56, 40: 58, 41: 50, 42: 42, 44: 51, 45: 14, 46: 23, 48: 7, 49: 47, 51: 31}
    ...
    if 64 - board.count('.') in playback_x if token == 'x' else 64 - board.count('.') in playback_o:
        print(playback_x[64 - board.count('.')] if token == 'x' else playback_o[64 - board.count('.')])
    elif MarksListing(board, token):
        print(MarksListing(board, token)[0])
    else:
        print('-1')
    return
    ...
    if board.count('.') < LIMIT_AB:
        best_move = alphabeta(board, token, -65, 65)
...
'68.4 71.0 71.2 69.8 70.7 73.5 73.5 75.6 75.8 75.1'
'5.0 5.0 5.7 5.2 5.2 5.7 7.3 17.1 47.1 213.0'
'69.9 69.9 70.2 71.5 72.7 67.9 70.6 72.8 74.5 74.3 76.9 76.6'
'5.0 5.1 5.4 5.2 5.4 6.41 7.2 9.3 17.4 42.8 91.9 291.5'