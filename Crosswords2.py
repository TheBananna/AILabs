...
def GetWordFromDescriptor(board, descriptor):
    if descriptor[1] - descriptor[0] + 1 == descriptor[2]:# check if horizontal word
        return board[descriptor[0] : descriptor[1] + 1]
...
def GetPossibleLetters(board, wordD, index):# returns the set of possible letters that can go in this word descriptor at this index
    ...
    if GetWordFromDescriptor(board, wordD).count('-') == wordD[2]:#if the entire word list is possible here, optimization, remove this somehow as it's a lot of list reads
    ...
    #if (search:=(''.join(board), tuple(wordD), index)) in possible_letter_cache:
    #    return possible_letter_cache[search]
    #cache checking done
    words = list(letter_index_length_dict[(letter, n, wordD[2])] for n, letter in enumerate(GetWordFromDescriptor(board, wordD)) if letter != '-')
    words.sort(key=lambda x: len(x))
    #possible_words = length_dict[(wordD[2])]# set this to the first set so that the expensive initial intersection doesn't need to happen, optimization
    #for n, letter in enumerate(GetWordFromDescriptor(board, wordD)):#get all of the possible words that could go here that have the already placed letters in it
    possible_words = words[0]
    for possibilities in words[1:]:
        possible_words = possible_words.intersection(possibilities)
        #if letter != '-':
        #    if (u:=(letter, n, wordD[2])) in letter_index_length_dict:
        #        possible_words = possible_words.intersection(letter_index_length_dict[u])
        #    else:
        #        continue
    #breakpoint()
    possible_letters = {word[index] for word in possible_words}#get the possible letters that coild go in this spot
...
#	 for each pair of words that share a letter in this position:
...
def PlaceWordsRecursively(board, placed_words, pos):
    global horizontals, verticals, word_list, positions_lut, best
    ...
    for intersection in horizontal_letters.intersection(vertical_letters):
        ...
        if (u:=board_copy.count('-')) < best:
            PrintBoard(board_copy, width, height)
            print('')
            best = u
        #if '-' not in (word:=GetWordFromDescriptor(board_copy, horiz)):#finished word
        if ''.join((word := GetWordFromDescriptor(board_copy, horiz))) in length_dict[horiz[2]]:
            if (word:=''.join(word)) in placed_words: continue#already used word
            placed_words_copy.add(word)
        if ''.join((word := GetWordFromDescriptor(board_copy, vert))) in length_dict[vert[2]]:
            if (word:=''.join(word)) in placed_words: continue
            placed_words_copy.add(word)
...
def main():
    global args, width, height, backwards, blocking_squares, seed_string_positions, word_list#blocking_squares will be used as the number of squares left to place
    backwards = []
    DEBUG = False
    if not args:
        DEBUG = True
