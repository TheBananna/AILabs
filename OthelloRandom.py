...
def ScoreBoardHeuristic(board, token):#attempts to play off of safe positions into safe positions. improvement could be to aggressively seek out safe positions when not having any. agggressive play to get corners would also be good
    ...
    score -= (sum([(board[s[1][0]] == token) + (board[s[1][1]] == token) for s in NEXT_TO_CORNERS.items() if board[s[0]] == '.']) - sum([(board[s[1][0]] == other_token) + (board[s[1][1]] == other_token) for s in NEXT_TO_CORNERS.items() if board[s[0]] == '.'])) * 50
...
def main():
    ...
    #args = ['...........................ox......xxx..........................', 'o']
    #args = ['.' * 27 + "ox......xo" + '.' * 27, 'x']
    #moves = '19181720211237292238_3_4_5_9_02633253410_11113_23031464716_824_6_7152314393240414842554344455253545057584956516059626163'.replace('_', '0')
    ...
    #if not moves:
        #print('-1')
        #print(f'min score {ScoreBoardAbsolute(board, token)}; move sequence: [-1]')
        #return
    #move = random.choice(moves)
    #new_board = Move(board, move, token)
    #PrintSnapshot(board, move, token)
    #print(f'{move}')
    #print(f'min score {ScoreBoardAbsolute(new_board, token)}; move sequence: [{move}]')
