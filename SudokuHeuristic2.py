...
def SolveSlotSet(positions, allowed, pzl):  # solves up to one position in the set, for now at least
    allowed_count = {char: 0 for char in char_set}
    forwards = False
    for pos in positions:
        if allowed[pos].__class__ != set:
            continue
        forwards = True
        for j in allowed[pos]:
            allowed_count[j] += 1
    if not forwards:#short circuit end if the equivalent set subtractions from before would have all yielded empty sets
        return False, -1, -1
    p = 0#for debugging
    for pos in positions:  # for every position in the subsection
        possibilities = allowed[pos]
        if possibilities.__class__ != set:
            continue
        for val in possibilities:
            if allowed_count[val] == 1:
                pzl[pos] = val  # will only replace 1 character per run through for now
                allowed[pos] = val
                return True, pos, val
    return False, -1, -1
...
def SolveMax(pzl, allowed):  # Solve puzzle as far as can go trivially heuristically
    global count
    count = 0#count of replaced dots
...
def HeuristicSolve(pzl, allowed):  # is not pure heuristic, when it runs out of puzzles with only one possibility in one of the positions it DFS's the options on the first smallest possibility position
    ...
    for temp in NeighborsHeuristic(pzl, allowed):
        ...
        if hs != '.':  # if hs isn't a . then it's a solution and should be propagated upwards
            backwards.append((pzl, loc_count))  # to track how the puzzle was solved, not great since multiple characters are inserted for every SolveMax call
            return hs
...
def main():
    ...
    if not args:
        ...
        #args = ['..............3.85..1.2.......5.7.....4...1...9.......5......73..2.1........4...9']
        #args = ['.4..5..67...1...4....2.....1..8..3........2...6...........4..5.3.....8..2........']
        args = ['..............3.85..1.2.......5.7.....4...1...9.......5......73..2.1........4...9']
        #args = ['puzzles.txt']
    if len(args[0]) < 60:  # file input
        ...
        for n, i in enumerate(reversed(backwards)):
            print(GetString(''.join(i[0])) + '\n' + str(i[1]) + '\n')
        if ns == '.':
            ns = args[0]

        # print(GetString(Puzzle(None, args[0])) + '\n')
        print(GetString(ns))
        print(len(backwards) + 1)
